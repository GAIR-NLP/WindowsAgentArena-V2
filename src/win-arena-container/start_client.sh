...
# exit 123
python run.py --agent "$agent" --model "$model" --max_steps "$max_steps" --som_origin "$som_origin" --a11y_backend "$a11y_backend" --worker_id "$worker_id" --num_workers "$num_workers" --result_dir "$result_dir" --test_all_meta_path "$json_name" --diff_lvl "$diff_lvl" --trial_id "$trial_id" --check_setup "$check_setup"