...
from mm_agents.uitars.prompt import (
    UITARS_ACTION_SPACE,
...
def parse_action_qwen2vl(plan_model, text, factor, image_height, image_width):
    text = text.strip()
    # 正则表达式匹配 Action 字符串
    ...
    action_str = action_str.replace("start_box='=", "start_box=").strip()
    ...
    for action_instance, raw_str in zip(parsed_actions, all_action):
        ...
        for param_name, param in params.items():
            ...
            if "start_box" in param_name or "end_box" in param_name:
                ...
                if "1.5" in plan_model:
                    float_numbers = []
                    for num_idx, num in enumerate(numbers):
                        num = float(num)
                        if (num_idx + 1) % 2 == 0:
                            float_numbers.append(float(num/720))
                        else:
                            float_numbers.append(float(num/1280))
                else:
    ...
    def predict(
        ...
        for turn, image in enumerate(self.history_images):
            ...
            if image.width * image.height > max_pixels:
                ...
                """
                print(f"Resize image: max_pixels = {max_pixels}, image pixels = {image.width} * {image.height} = {image.width * image.height}")
                resize_factor = math.sqrt(max_pixels / (image.width * image.height))
                width, height = int(image.width * resize_factor), int(image.height * resize_factor)
                print(f"Resize image to: {width} * {height}")
                image = image.resize((width, height))
            if image.width * image.height < min_pixels:
                print(f"Resize image: min_pixels = {min_pixels}, image pixels = {image.width} * {image.height} = {image.width * image.height}")
        ...
        while True:
            try:
                ...
                if try_times <= 0:
                    raise ValueError(f"No valid action parsed from client after 5 tries.")
            
            except Exception as e:
                print(f"Error when getting response from client: {e}")
                # 随机睡 1-5 秒
                time.sleep(random.randint(1, 5))

                if try_times <= 0:
                    raise RuntimeError(f"Error when getting response from client after 5 tries: {e}")
